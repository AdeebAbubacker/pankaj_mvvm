...
class _AchievmentsCardState extends State<AchievmentsCard> {
    ...
    setState(() {
      ...
      Hive.box<AchievmentDB>('achievmentBox').put(
        ...
        AchievmentDB(
          ...
          category: category,
          uploadfile: filePaths[currentKey],
        ),
      );
  ...
  void _updateHiveData(int key) {
     final String category = selectedDropdownValues[key] != null ? selectedDropdownValues[key]! : 'Nothing Selected';
    Hive.box<AchievmentDB>('achievmentBox').put(
      key,
      AchievmentDB(
        achievmentController: controllers[key]?.text ?? 'No Category',
        category: category ?? 'No Category',
        uploadfile: filePaths[key],
      ),
    );
  }
        ...
        Container(
          ...
          child: Column(
            ...
            FilePickerScreen(
                onFileSelected: (filePath) {
                  setState(() {
                    selectedFilePath = filePath; filePaths[key] = filePath;
                  });
                },
              ),
            ],
