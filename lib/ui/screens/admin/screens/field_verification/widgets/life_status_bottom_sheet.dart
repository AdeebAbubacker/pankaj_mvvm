...
class _LifeStatusBottomSheetState extends State<LifeStatusBottomSheet> {
  late Box<LifeStatusDB> lifestatusInfoBox;
  ...
  Future<void> setuplifestatusInfoBox() async {
    ...
    displayedBanks = lifestatusInfoBox.values.map((bank) => bank.name).toList();

    // Add a listener to update the displayed banks when data changes in Hive
    lifestatusInfoBox.listenable().addListener(() {
      if (mounted) {
        setState(() {
          displayedBanks =
              lifestatusInfoBox.values.map((bank) => bank.name).toList();
        });
      }
    });
                              ...
                              return InkWell(
                                ...
                                onTap: () {
                                  if (newDisplayedBanks.isNotEmpty &&
                                    ...
                                    final selectedBankObject = lifestatusInfoBox
                                        .values
                                        .firstWhere((bank) =>
  ...
  void _updateDisplayedBanks(String searchTerm) {
    final List<String> updatedDisplayedBanks = lifestatusInfoBox.values
        .where((bank) =>
