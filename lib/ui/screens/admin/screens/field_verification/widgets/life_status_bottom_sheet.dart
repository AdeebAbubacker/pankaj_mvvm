...
class LifeStatusBottomSheet extends StatefulWidget {
  ...
  @override
  State<LifeStatusBottomSheet> createState() => _LifeStatusBottomSheetState();
}

class _LifeStatusBottomSheetState extends State<LifeStatusBottomSheet> {
  late Box<LifeStatusDB> life_status_box;
  ...
  Future<void> setuplife_status_box() async {
  life_status_box = await Hive.openBox<LifeStatusDB>('life_status_box');

  // Initialize displayedBanks with the initial values from Hive
  displayedBanks = life_status_box.values.map((bank) => bank.name).toList();

  // Add a listener to update the displayed banks when data changes in Hive
  life_status_box.listenable().addListener(() {
    if (mounted) {
      setState(() {
        displayedBanks = life_status_box.values.map((bank) => bank.name).toList();
      });
                              ...
                              return InkWell(
                                ...
                                onTap: () {
                                  if (newDisplayedBanks.isNotEmpty &&
                                    ...
                                    final selectedBankObject = life_status_box.values
                                        .firstWhere((bank) =>
  ...
  void _updateDisplayedBanks(String searchTerm) {
    final List<String> updatedDisplayedBanks = life_status_box.values
        .where((bank) =>
