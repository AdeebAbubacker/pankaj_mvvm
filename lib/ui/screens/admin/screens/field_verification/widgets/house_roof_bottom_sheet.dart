...
class HouseRoofBottomSheet extends StatefulWidget {
  final bottomSheetheight;
  final String title;
  final hintText;

  HouseRoofBottomSheet({
    Key? key,
    required this.title,
  ...
  @override
  State<HouseRoofBottomSheet> createState() => _HouseRoofBottomSheetState();
}

class _HouseRoofBottomSheetState extends State<HouseRoofBottomSheet> {
  late Box<HouseRoofDB> houseroofBox;
  ...
  Future<void> setuplifestatusInfoBox() async {
    ...
    displayedBanks = houseroofBox.values.map((bank) => bank.name).toList();

    // Add a listener to update the displayed banks when data changes in Hive
    houseroofBox.listenable().addListener(() {
      if (mounted) {
        setState(() {
          displayedBanks =
              houseroofBox.values.map((bank) => bank.name).toList();
        });
      }
    });
                              ...
                              return InkWell(
                                ...
                                onTap: () {
                                  if (newDisplayedBanks.isNotEmpty &&
                                    ...
                                    final selectedBankObject = houseroofBox
                                        .values
                                        .firstWhere((bank) =>
  ...
  void _updateDisplayedBanks(String searchTerm) {
    final List<String> updatedDisplayedBanks = houseroofBox.values
        .where((bank) =>
