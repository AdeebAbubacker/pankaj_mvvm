...
class WaterSourceBottomSheet extends StatefulWidget {
  final bottomSheetheight;
  final String title;
  final hintText;

  WaterSourceBottomSheet({
    Key? key,
    required this.title,
  ...
  @override
  State<WaterSourceBottomSheet> createState() => _WaterSourceBottomSheetState();
}

class _WaterSourceBottomSheetState extends State<WaterSourceBottomSheet> {
  late Box<WaterSourceDB> waterSourceBox;
  ...
  Future<void> setuplifestatusInfoBox() async {
    ...
    displayedBanks = waterSourceBox.values.map((bank) => bank.name).toList();

    // Add a listener to update the displayed banks when data changes in Hive
    waterSourceBox.listenable().addListener(() {
      if (mounted) {
        setState(() {
          displayedBanks =
              waterSourceBox.values.map((bank) => bank.name).toList();
        });
      }
    });
                              ...
                              return InkWell(
                                ...
                                onTap: () {
                                  if (newDisplayedBanks.isNotEmpty &&
                                    ...
                                    final selectedBankObject = waterSourceBox
                                        .values
                                        .firstWhere((bank) =>
  ...
  void _updateDisplayedBanks(String searchTerm) {
    final List<String> updatedDisplayedBanks = waterSourceBox.values
        .where((bank) =>
