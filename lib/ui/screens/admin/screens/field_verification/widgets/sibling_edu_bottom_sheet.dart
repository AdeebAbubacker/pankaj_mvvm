...
class SiblingEducationalBottomSheet extends StatefulWidget {
  final bottomSheetheight;
  final String title;
  final hintText;

  SiblingEducationalBottomSheet({
    Key? key,
    required this.title,
  ...
  @override
  State<SiblingEducationalBottomSheet> createState() => _SiblingEducationalBottomSheetState();
}

class _SiblingEducationalBottomSheetState extends State<SiblingEducationalBottomSheet> {
  late Box<SiblingEducationDB> siblingeducationbox;
  ...
  Future<void> setuplifestatusInfoBox() async {
    siblingeducationbox = await Hive.openBox<SiblingEducationDB>('siblingeducationbox');

    // Initialize displayedBanks with the initial values from Hive
    displayedBanks = siblingeducationbox.values.map((bank) => bank.name).toList();

    // Add a listener to update the displayed banks when data changes in Hive
    siblingeducationbox.listenable().addListener(() {
      if (mounted) {
        setState(() {
          displayedBanks =
              siblingeducationbox.values.map((bank) => bank.name).toList();
        });
      }
    });
                              ...
                              return InkWell(
                                ...
                                onTap: () {
                                  if (newDisplayedBanks.isNotEmpty &&
                                    ...
                                    final selectedBankObject = siblingeducationbox
                                        .values
                                        .firstWhere((bank) =>
  ...
  void _updateDisplayedBanks(String searchTerm) {
    final List<String> updatedDisplayedBanks = siblingeducationbox.values
        .where((bank) =>
  ...
  Widget build(BuildContext context) {
    ...
    return BlocBuilder<SelctedbankBloc, SelctedbankState>(
