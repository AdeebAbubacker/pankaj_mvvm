...
class HousePlotSizeBottomSheet extends StatefulWidget {
  ...
  @override
  State<HousePlotSizeBottomSheet> createState() => _HousePlotSizeBottomSheetState();
}

class _HousePlotSizeBottomSheetState extends State<HousePlotSizeBottomSheet> {
  late Box<HousePlotSizeDB> houseplotsizeBox;
  ...
  Future<void> setuplifestatusInfoBox() async {
    ...
    displayedBanks = houseplotsizeBox.values.map((bank) => bank.name).toList();

    // Add a listener to update the displayed banks when data changes in Hive
    houseplotsizeBox.listenable().addListener(() {
      if (mounted) {
        setState(() {
          displayedBanks =
              houseplotsizeBox.values.map((bank) => bank.name).toList();
        });
      }
    });
                              ...
                              return InkWell(
                                ...
                                onTap: () {
                                  if (newDisplayedBanks.isNotEmpty &&
                                    ...
                                    final selectedBankObject = houseplotsizeBox
                                        .values
                                        .firstWhere((bank) =>
  ...
  void _updateDisplayedBanks(String searchTerm) {
    final List<String> updatedDisplayedBanks = houseplotsizeBox.values
        .where((bank) =>
