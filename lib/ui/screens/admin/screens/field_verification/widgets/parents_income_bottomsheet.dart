...
class ParentsIncomeBottomSheet extends StatefulWidget {
  ...
  @override
  State<ParentsIncomeBottomSheet> createState() =>
      _ParentsIncomeBottomSheetState();
}

class _ParentsIncomeBottomSheetState
    extends State<ParentsIncomeBottomSheet> {
  late Box<ParentsIncomeDB> parentsIncomeBox;
  ...
  Future<void> setupparentsIncomeBox() async {
    parentsIncomeBox =
        await Hive.openBox<ParentsIncomeDB>('parentsIncomeBox');

    // Initialize displayedBanks with the initial values from Hive
    displayedBanks =
        parentsIncomeBox.values.map((bank) => bank.name).toList();

    // Add a listener to update the displayed banks when data changes in Hive
    parentsIncomeBox.listenable().addListener(() {
      if (mounted) {
        setState(() {
          displayedBanks =
              parentsIncomeBox.values.map((bank) => bank.name).toList();
                                ...
                                onTap: () {
                                  if (newDisplayedBanks.isNotEmpty &&
                                    ...
                                    final selectedBankObject =
                                        parentsIncomeBox.values.firstWhere(
                                            (bank) =>
  ...
  void _updateDisplayedBanks(String searchTerm) {
    final List<String> updatedDisplayedBanks = parentsIncomeBox.values
        .where((bank) =>
