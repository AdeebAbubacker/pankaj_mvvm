...
class HouseRentalBottomSheet extends StatefulWidget {
  final bottomSheetheight;
  final String title;
  final hintText;

  HouseRentalBottomSheet({
    Key? key,
    required this.title,
  ...
  @override
  State<HouseRentalBottomSheet> createState() => _HouseRentalBottomSheetState();
}

class _HouseRentalBottomSheetState extends State<HouseRentalBottomSheet> {
  late Box<RentalHouseDB> rentalHouseBox;
  ...
  Future<void> setuplifestatusInfoBox() async {
    ...
    displayedBanks = rentalHouseBox.values.map((bank) => bank.name).toList();

    // Add a listener to update the displayed banks when data changes in Hive
    rentalHouseBox.listenable().addListener(() {
      if (mounted) {
        setState(() {
          displayedBanks =
              rentalHouseBox.values.map((bank) => bank.name).toList();
        });
      }
    });
                              ...
                              return InkWell(
                                ...
                                onTap: () {
                                  if (newDisplayedBanks.isNotEmpty &&
                                    ...
                                    final selectedBankObject = rentalHouseBox
                                        .values
                                        .firstWhere((bank) =>
  ...
  void _updateDisplayedBanks(String searchTerm) {
    final List<String> updatedDisplayedBanks = rentalHouseBox.values
        .where((bank) =>
