...
class HousePlasterBottomSheet extends StatefulWidget {
  ...
  @override
  State<HousePlasterBottomSheet> createState() => _HousePlasterBottomSheetState();
}

class _HousePlasterBottomSheetState extends State<HousePlasterBottomSheet> {
  late Box<HousePlasterDB> houseplasterBox;
  ...
  Future<void> setuplifestatusInfoBox() async {
    ...
    displayedBanks = houseplasterBox.values.map((bank) => bank.name).toList();

    // Add a listener to update the displayed banks when data changes in Hive
    houseplasterBox.listenable().addListener(() {
      if (mounted) {
        setState(() {
          displayedBanks =
              houseplasterBox.values.map((bank) => bank.name).toList();
        });
      }
    });
                              ...
                              return InkWell(
                                ...
                                onTap: () {
                                  if (newDisplayedBanks.isNotEmpty &&
                                    ...
                                    final selectedBankObject = houseplasterBox
                                        .values
                                        .firstWhere((bank) =>
  ...
  void _updateDisplayedBanks(String searchTerm) {
    final List<String> updatedDisplayedBanks = houseplasterBox.values
        .where((bank) =>
