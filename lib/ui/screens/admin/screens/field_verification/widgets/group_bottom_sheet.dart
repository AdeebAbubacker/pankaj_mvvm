...
class SchoolGroupBottomSheet extends StatefulWidget {
  ...
  @override
  State<SchoolGroupBottomSheet> createState() => _SchoolGroupBottomSheetState();
}

class _SchoolGroupBottomSheetState extends State<SchoolGroupBottomSheet> {
  late Box<SchoolGroupDB> school_group_box;
  ...
  Future<void> setupschool_group_box() async {
  ...
  displayedBanks = school_group_box.values.map((bank) => bank.name).toList();

  // Add a listener to update the displayed banks when data changes in Hive
  school_group_box.listenable().addListener(() {
    if (mounted) {
      setState(() {
        displayedBanks = school_group_box.values.map((bank) => bank.name).toList();
      });
                              ...
                              return InkWell(
                                ...
                                onTap: () {
                                  if (newDisplayedBanks.isNotEmpty &&
                                    ...
                                    final selectedBankObject = school_group_box.values
                                        .firstWhere((bank) =>
  ...
  void _updateDisplayedBanks(String searchTerm) {
    final List<String> updatedDisplayedBanks = school_group_box.values
        .where((bank) =>
